...
public class BookOrderControllerMVCTest {
	...
	/*
	 * @Test public void TestCreateBook() throws Exception {
	 * when(service.addBook(order)).thenReturn(order); ObjectMapper mapper = new
	 * ObjectMapper(); String jsonbody = mapper.writeValueAsString(order);
	...
	/*
	 * @Test public void TestUpdateBookOrder() throws Exception {
	 * when(service.findId("1")).thenReturn(order); when(service.updateId("1",
	 * order)).thenReturn(order);
	 * 
	 * ObjectMapper mapper = new ObjectMapper(); String jsonbody =
